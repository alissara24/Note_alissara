...
function HomePage() {
  ...
  const getData = () => {
    axios
      .get("http://localhost:5002/notedetail")
  ...
  const handleCreateNote = (evt) => {
    ...
    axios.post(`http://localhost:5002/notedetail`, submitData).then((res) => {
      setStatus(true);
      setNewNoteText(null);
    });
  ...
  const handleUpdateNote = () => {
    ...
    axios
      .put(`http://localhost:5002/notedetail/${idNoteText}`, submitData)
      .then(function (response) {
        setNewNoteText(null);
        setIdNoteText(null);
        getData();
  ...
  const deleteDetail = (e) => {
    ...
    if (confirm(text) == true) {
      axios
        .delete(`http://localhost:5002/notedetail/${e}`)
        .then(function (response) {
  ...
  return (
    <div
      style={{ backgroundColor: "#92CEA8", height: "100%", minHeight: "100vh" }}
    >
                ...
                >
                  <p style={{ paddingLeft: 20 }}>
                    ...
                    >
                      <button
                        onClick={(e) =>
                          EditDetail([note._id, note.note_detail])
                        }
                      ...
                      <button
                        onClick={(e) => deleteDetail(note._id)}
                        style={{
                          marginLeft: 20,
                          marginBottom: 10,
                          backgroundColor: "red",
